...
traverse(ast, {
    VariableDeclaration(path) {
        ...
        if (declarations.length === 3) {
            if (declarations[0].init && declarations[0].init.object?.name === string_func_name) {
                // console.log(path + "");
                ...
                if (bingding1 && bingding1.constant) {
                    bingding1.referencePaths.forEach(refPath => {
                        ...
                        if (types.isCallExpression(refParentPath.node) &&
...
const out_code = generator(ast, {compact: false, jsescOption: {minimal: true,}}).code;
