...
## 一、this 问题详解
其实这是个 this 指向问题, new Proxy(target, handler) 会返回一个代理对象objProxy, 代理对象objProxy 调用函数时的 this 会指向 objProxy 肯定不是 target;
其实就看怎么使用了, 理解好这个 this 指向可以更好的使用吐环境脚本;
watch 函数目的是监控属性读写，不需要 getter 里的 this 重定向到 proxy, 所以不传 receiver 反而更符合"透明监控"的语义
### 1.1 问题现象
...
const proxy = new Proxy(target, {
    get(target, property, receiver) {
        ...
        return value;  // 问题：函数被返回后，this 不再是 target,  this 绑定取决于调用时的方式，而不是取出方式。
    }
});
...
```javascript
get(target, property, receiver) {
    // receiver 指代理对象, 如果传了 receiver, 则 this 会指向 receiver 而不是 target
...
#### 3.2.2 Symbol 属性
Symbol.toStringTag 控制 Object.prototype.toString.call(obj) 的返回值, 返回值里 object 后面那个词，就是 Symbol.toStringTag 决定的
